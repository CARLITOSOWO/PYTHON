import asyncio # AYUDA CON EL ASINCRONISMO 
import time # SIMULA EL TIEMPO DE ESPERA 
import random # SIMULA LOS DATOA ALEATORIOS 
import multiprocessing 

#Función asíncrona para verificar el inventario SIMULA LA ENTRADA Y SALIDA CON UNA BASE DE DATOS 
async def check_inventory(item): # CREAMOS UN PROCESO ASINCRONO 
    print(f'Verificando inventario para {item}...')
    await asyncio.sleep(random.randint(3,6)) # PAUSAMOS EL PROCESAMIENTO PARA INCIAR OTRO PROCESO 
...
def calculate_total(items):
    print(f'Calculando el costo total para {len(items)} articulos...') # len CUENTA EL NUMERO DE ELEMETOS DE UNA LISTA 
    time.sleep(5)
...
async def process_order(order_id, items):
    ...
    #Verificar inventario para cada artículo Y O VAMOS HACER ASINCRONAMENTE
    inventory_checks = [check_inventory(item['name']) for item in items]
    inventory_results = await asyncio.gather(*inventory_checks) # gather ->

    if not all(inventory_results):
        print(f'Orden {order_id} cancelada: Producto no está disponible')

    with multiprocessing.Pool() as pool: # PARALELISAMOS EL PROCESO CON EL pool 
...
async def main(): #FUNCION PRINCIPAL 
    orders = [
...
"""
-> gather NOS AYUDA A TRABAJAR CON TODAS LAS TAREAS PERO DE MANERA CONCURRENTE 
LO QUE HACE ES LANZAR EL PROCESAMIENTO DE TODAS LAS TAREAS DE MANERA SIMULTANEA 
